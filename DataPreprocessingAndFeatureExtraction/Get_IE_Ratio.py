...
def Calculate_IE_Ratio(data):
    ...
    #print(len(r_pvs),len(r_vals),len(r_hts),len(r_time))
    I_E=[]
    Start=[]
    End=[]
    I_only=[]
    ...
    while(i<data.shape[0]-4):
        ...
        if((r_pvs[i]==-1) and r_pvs[i+1]==1 and r_pvs[i+2]==-1):
            ...
            if numo==0:
                print("HAHHAHAHAHAHHAHAHAHHAHAHAH")
                print(r_time[i])
                print("HAHHAHAHAHAHHAHAHAHHAHAHAH")
            numo=numo+1
    ...
    #d = {'Start':Start,'End':End,'I_E':I_E,'I_only':I_only,'E_only':E_only,\
     #       'I_by_Total':I_by_Total,'Total_IE':Total_IE,'Total_IE2':Total_IE2,'Diff_IE':Diff_IE,"count":count}
